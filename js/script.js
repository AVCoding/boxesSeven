...
$(document).ready(function(){
    ...
    //   $(this).find('.more-options').toggleClass('show');
    ...
    function updateDroppables(){
        $( ".open-box-wrapper" ).droppable({
            ...
            drop: function(event, ui ) {
              ...
              $('.order-wrapper p').css('font-size', '11.5px');
              $('.order-wrapper span').css('font-size', '8px');

              var draggableItemTitle = $(ui.draggable).parents('.open-box-wrapper').find('.title');

              if ($(this).hasClass('main-box')){

                if ($(this).find('.main-item[data-item="' + $(ui.draggable).data('item') + '"]').length < 1 ){
                  $(this).find('.main-open-box').append($(ui.draggable).addClass('main-item'));
                }

                else if ($(this).find('.main-item[data-item="' + $(ui.draggable).data('item') + '"]').length == 1 ){
                  $(this).find('.main-item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text( parseInt( $(this).find('.main-item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text() ) + parseInt( $(ui.draggable).find('.total').text() ) );
                  $(ui.draggable).remove();
                }
              ...
              if (!$(this).hasClass('main-box')) {
                  // console.log($(ui.draggable).siblings('.main-item').length);

                  if (!$(ui.draggable).parents('.open-box-wrapper').hasClass('main-box') ) {
                    if ($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length == 1 ){
                      // console.log($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('total').text());
                      $(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text( parseInt( $(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text() ) + parseInt( $(ui.draggable).find('.total').text() ) );
                      $(ui.draggable).remove();
                     }
                     if ($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length < 1 ){
                  ...
                  if ($(ui.draggable).siblings('.main-item').length > 1) {

                    if ($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length == 1 ) {
                      $(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text(  parseInt($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text() ) + parseInt( $(ui.draggable).find('.total').text() ) );
                      $(ui.draggable).remove();
                    }
                    // console.log($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length);
                    if ($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length < 1 )
                    {
                      $(this).find('.open-box').append($(ui.draggable).removeClass('main-item'));
                    }                   
                  }

                  if ($(ui.draggable).siblings('.main-item').length == 1){
                    if ($(ui.draggable).find('.total').text() > 1 ) {
                      if ($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length == 1 ) {
                        $(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text(  parseInt($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text() ) + (parseInt( $(ui.draggable).find('.total').text() ) - 1 ) );
                        $(ui.draggable).find('.total').text(1);
                      }
                      if ($(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').length < 1 )
                      {
                        $(this).find('.open-box').append($(ui.draggable).clone().draggable({
                                                              revert: "invalid",
                                                              containment: "document",
                                                              helper: "clone",
                                                              cursor: "move"
                                                            }).removeClass('main-item') ) ;
                         $(this).find('.item[data-item="' + $(ui.draggable).data('item') + '"]').find('.total').text(   (parseInt($(ui.draggable).find('.total').text()) ) - 1   );
                        $(ui.draggable).find('.total').text(1);
              ...
              $('.open-box-wrapper').each(function(){
                ...
                if (parseInt(ui.draggable.siblings('.main-item').length) == 1) {
                  ui.helper.siblings('.main-item').length = 50;
                }
                // console.log(ui.helper.siblings('.main-item').length-1);
              ...
              // console.log((($(this).siblings('.main-box').find('.main-item').length - 1 ) ));
